            ...
            if (isset($input['categoty']) && !empty($input['categoty'])) {
                ...
                foreach ($input['categoty'] as $key => $val) {
                    ...
                    if ($input['available_quantity'][$key] != '') {
                        $check_update = $this->project_cost_model->get_stock_history_by_id($input['quotation']['job_id'],$input['product_id'][$key]);
                        if ($input['old_quantity'][$key] != $input['quantity'][$key]) {
                            if ($input['old_quantity'][$key] > $input['quantity'][$key]) {
                                ...
                                if($check_update>0)
                                     ...
                                     $insertss['quantity'] = ($input['available_quantity'][$key] -$input['quantity'][$key]);
                                $inv_id['diff'] = $dfff;
                ...
                $product = $this->project_cost_model->get_pcdetails_by_productid($input['quotation']['q_id'],$project_cost_id,implode(',', $input['product_id']));
                foreach ($product as $data){
                    $check = $this->project_cost_model->get_stock_history_by_id($input['quotation']['job_id'],$data['product_id']);
                    if($check>0){
                        $dfff = $data['quantity'];
                        $pro_ins['quantity'] = $data['available_quantity'] + $dfff;
                        $pro_inv_id['diff'] = $dfff;
            
                        $pro_ins['category'] = $data['category'];
                        $pro_ins['brand'] = $data['brand'];
                        $pro_ins['product_id'] = $data['product_id'];
                        $pro_inv_id['inv_id'] = $input['quotation']['job_id'];
                        $this->project_cost_model->update_stock($pro_ins, $pro_inv_id);
                    }
                }
...
//        exit;
        ...
        $check_update = $this->project_cost_model->get_stock_history_by_id($quotation[0]['job_id']);
        if($check_update>0)
            $datas['is_update'] = 1;
        else  
            $datas['is_update'] = 0;
