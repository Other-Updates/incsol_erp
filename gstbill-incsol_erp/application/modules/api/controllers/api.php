...
class Api extends REST_Controller {
    ...
    public function pending_leads_post() {
        ...
        if (!empty($data_input)) {
            if (!empty($data_input['customer_id'])) {
                $customer_id = $data_input['customer_id'];
                $pending_leads = $this->api_model->get_non_completed_leads($customer_id);
                if (!empty($pending_leads)) {
                    $output = array('status' => 'success', 'message' => 'Not Completed Leads', 'data' => $pending_leads);
                    $this->response($output);
                } else {
                    $output = array('status' => 'false', 'message' => 'Data Not Found');
                    $this->response($output);
    ...
    function get_service_history_post() {

        $data_input = $this->_get_custom_post_values(); // JSON Input
        $customer_id = $data_input['customer_id'];
        $employee_id = $data_input['emp_id'];
        $service_id = $data_input['service_id'];
        if (!empty($data_input['service_type']) && $service_id > 0 && $customer_id > 0 && $employee_id > 0) {
            $data["service"] = $this->api_model->get_service_history($customer_id,$employee_id,$service_id,$type);
            if (!empty($data["service"])) {
                $output = array('status' => 'success', 'message' => 'Service History successfully found', 'data' => $data["service"]);
                $this->response($output);
            } else {
                $output = array('status' => 'false', 'message' => 'Service History Not Found');
                $this->response($output);
            }
        } else {
            $output = array('status' => 'false', 'message' => 'Fields Missing.');
            $this->response($output);
        }
    }
    ...
    function get_service_pending_list_post() {
        ...
        if (!empty($data_input['service_type'])) {
            ...
            if ($type == 'customer') {
                $data["service"] = $this->api_model->get_service_not_completed_list($customer_id, $type);
            } else if ($type == 'employee') {
                $from_date = $data_input['from_date'];
                $to_date = $data_input['to_date'];
                $data["service"] = $this->api_model->get_service_not_completed_list($employee_id, $type, $from_date, $to_date);
            }
            if (!empty($data["service"])) {
                $output = array('status' => 'success', 'message' => 'Service List successfully found', 'data' => $data["service"]);
                $this->response($output);
    ...
    function get_pending_service_list_post() {
        $data_input = $this->_get_custom_post_values(); // JSON Input
        $customer_id = $data_input['customer_id'];
        $data["pending"] = $this->api_model->get_not_completed_service_list($customer_id);
